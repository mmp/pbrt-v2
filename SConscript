...
Import('env', 'use_sdl', 'parallel_libs', 'exr_libs', 'tiff_libs')
...
if (use_sdl):
    Import('sdl_main')
    renderer_src = renderer_src + sdl_main

core_src = [ 'core/api.cpp',
             'core/camera.cpp',
             'core/diffgeom.cpp',
             'core/error.cpp',
             'core/film.cpp',
...
filters_src = [ 'filters/box.cpp',
                'filters/gaussian.cpp', 
                'filters/mitchell.cpp', 
                'filters/sinc.cpp',
                'filters/triangle.cpp' ]
integrators_src = [ 'integrators/ambientocclusion.cpp',
...
lights_src = [ 'lights/diffuse.cpp',
               ...
               'lights/spot.cpp' ]
materials_src = [ 'materials/glass.cpp', 
...
textures_src = [ 'textures/bilerp.cpp',
...
volumes_src = [ 'volumes/exponential.cpp',
...
if arch != 'win32' and len(exr_libs) > 0:
    output['tifftoexr'] = env.Program('tifftoexr', [ 'tools/tifftoexr.cpp' ], 
                                      LIBS = env_libs + tiff_libs + exr_libs)
    output['exrtotiff'] = env.Program('exrtotiff', [ 'tools/exrtotiff.cpp' ], 
                                      LIBS = env_libs + exr_libs + tiff_libs)
    output['defaults'] = output['defaults'] + [ output['exrtotiff'], output['tifftoexr'] ]
